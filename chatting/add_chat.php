...
if(isset($_POST['insertData'])){
  ...
  // if chat manager have the data
  $message = $_POST["mxg"];
  $insert = "INSERT INTO `groupchat`(`user_id`,`admin_id`, `chatter_name`,`chat_id`, `message`) VALUES ('$id', '$admin_id', 'user', '$chat_id','$message')";
  ...
  // if chat manager have the data 
  $message2 = $_POST["mxg"];
  $insert2 = "INSERT INTO `groupchat`(`user_id`,`admin_id`, `chatter_name`,`chat_id`, `message`) VALUES ('$id', '$admin_id', 'user', '$chat_id','$message2')";
...
if(isset($_GET['selectData'])){
        ...
        // echo $admin;
        // $checkadmin = ''
        $sel = "SELECT * FROM `chat_manager` WHERE `admin_id` = '$admin' AND `user_id` = '$id'";
        ...
        if ($row > 0){
          ...
          $sel3 = "SELECT * FROM `admins` WHERE id='$admin'";
          $exe3= mysqli_query($conn,$sel3);
          $row3 = mysqli_fetch_array($exe3);
          if($chatter_name == 'user'){
          echo '<p class="mex admin_chat bg-success text-white">'.$row1['username'].' : '.$row2['message'].'</p>';
          }else{
          echo '<p class="mex bg-primary text-white">'.$row3['username'].' : '.$row2['message'].'</p>';
